"""Shared style, palette and data readers for the report figures.

Every figure in prelim_design_report.html is generated by a script in this
directory. Nothing here is hand-drawn or traced: each figure either reads a
solver output file (VSPAERO .lod/.polar, SU2 surface CSV, OpenVSP wave-drag
CSV) or is constructed from the same closed-form geometry the OpenVSP build
scripts use.

Palette: the validated categorical order (blue, orange, aqua, yellow, magenta,
green, violet, red), assigned in fixed slot order and never cycled. Status
colours are reserved for pass/fail marks and never used as a series.
"""
import os
import re
...
def read_lod(path):
    """VSPAERO .lod -> (list of per-case section tables, reference dict).

    The .lod repeats the whole spanwise table once per solved case, keyed by
    the leading 'Iter' column changing back to the first value. Returns one
    dict-of-arrays per case, in solve order.
    """
...
def read_wavedrag_area(path):
    """OpenVSP wave-drag CSV -> (x [ft], mean cross-sectional area [ft^2]).

    The file stores one Slice_Area row per Mach-cone rotation angle; the
    area-rule curve is the average over rotations (Whitcomb's equivalent body).
    """
...
# --------------------------------------------------------- full_v7 geometry ---
# The station table below is copied verbatim from the FUSE list in
# openvsp/vspaero_runs/11_full_v7/gen_v7.py, so every outline drawn here is the
# same body OpenVSP lofted and VSPAERO solved.
FUSE_L = 42.2
FUSE_STATIONS = [
    # x,      z,       height, width
